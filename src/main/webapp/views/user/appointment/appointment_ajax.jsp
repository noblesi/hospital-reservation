...
for (int i = 0; i < totalCnt; i++) {
    if (i % 9 == 0) { out.print("<div class='sliderPage'><table class='slTab'>"); }
    if (i % 3 == 0) { out.print("<tr class='slRow'>"); }
    
    DepartmentDTO dept = deptList.get(i);
    
    out.print("<td class='slCol'>");
    out.print("  <input class='deptRadio' style='display: none;' type='radio' name='dept' value='" + dept.getDeptNo() + "' id='" + dept.getDeptNo() + "'>"); 
    out.print("  <label for='" + dept.getDeptNo() + "'>" + dept.getDeptName() + "</label>");
    out.print("</td>");
    
    if (i % 3 == 2 || i == totalCnt - 1) { out.print("</tr>"); }
    if (i % 9 == 8 || i == totalCnt - 1) { out.print("</table></div>"); }
}
%>