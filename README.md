...
│   │   ├── DoctorStatusDTO.java
│   │   ├── DoctorPositionDTO.java
│   │   ├── DoctorCareerDTO.java
│   │   └── DoctorEducationDTO.java
│   └── util
│       ├── DBConnection.java
│       ├── GetKey.java
│       └── PaginationUtil.java
...
│   │   └── AdminDashboardChartDTO.java
│   ├── member
│   │   ├── controller
│   │   │   └── AdminMemberListServlet.java
│   │   ├── AdminMemberService.java
│   │   ├── AdminMemberDAO.java
│   │   ├── AdminMemberDetailServlet.java
│   │   └── dto
│   │       └── AdminMemberSearchDTO.java
│   ├── appointment
│   │   ├── controller
│   │   │   ├── AdminAppointmentListServlet.java
│   │   │   └── AdminAppointmentStatusServlet.java
...
│   ├── memo
│   │   ├── controller
│   │   │   ├── AdminMemoSaveServlet.java
│   │   │   └── AdminMemoDeleteServlet.java
...
│   ├── doctor
│   │   ├── controller
│   │   │   ├── AdminDoctorListServlet.java
│   │   │   └── AdminDoctorFormServlet.java
│   │   ├── AdminDoctorService.java
│   │   ├── AdminDoctorDAO.java
│   │   └── dto
│   │       ├── AdminDoctorFormDTO.java
│   │       ├── AdminDoctorFormOptionDTO.java
│   │       └── AdminDoctorSearchDTO.java
│   ├── department
│   │   ├── controller
│   │   │   ├── AdminDepartmentListServlet.java
│   │   │   └── AdminDepartmentFormServlet.java
│   │   ├── AdminDepartmentService.java
│   │   ├── AdminDepartmentDAO.java
│   │   └── dto
│   │       └── AdminDepartmentSearchDTO.java
│   └── board
│       ├── controller
│       │   ├── AdminBoardListServlet.java
│       │   ├── AdminBoardFormServlet.java
│       │   ├── AdminBoardSaveServlet.java
│       │   └── AdminBoardDeleteServlet.java
...
├── member
│   ├── controller
│   │   ├── MemberLoginProcessServlet.java
│   │   ├── MemberJoinProcessServlet.java
│   │   ├── MemberFindIdProcessServlet.java
│   │   ├── MemberFindPasswordProcessServlet.java
│   │   ├── MemberResetPasswordProcessServlet.java
│   │   ├── MemberMyPageServlet.java
│   │   ├── MemberMyPageInfoServlet.java
│   │   ├── MemberPasswordCheckServlet.java
│   │   ├── MemberUpdateInfoServlet.java
│   │   ├── MemberWithdrawalPasswordCheckServlet.java
│   │   ├── MemberWithdrawProcessServlet.java
│   │   ├── MemberCancelWithdrawalServlet.java
│   │   ├── MemberCancelAppointmentServlet.java
│   │   └── MemberLogoutProcessServlet.java
│   ├── LoginService.java
│   ├── LoginDAO.java
...
│   └── dto
│       ├── FindAccountDTO.java
│       ├── UserAppointmentDTO.java
│       └── UserMedicalRecordDTO.java
└── user
    ...
    └── appointment
        ├── controller
        │   ├── UserAppointmentReserveServlet.java
        │   ├── UserAppointmentAjaxServlet.java
        │   ├── UserAppointmentProcessServlet.java
        │   ├── UserAppointmentListServlet.java
        │   ├── UserAppointmentCancelServlet.java
        │   ├── UserAppointmentSuccessServlet.java
        │   └── UserAppointmentSessionUtil.java
...
- DAO와 Service는 `admin`, `member`, `user` 아래의 기능별 폴더에 작성한다.
- 회원/인증/마이페이지 기능은 현재 `com.hospital.member`와 `views/member`를 기준으로 작성되어 있다.
...
| 채대경 | 의료진/진료과 관리 | `AdminDoctorService`, `AdminDoctorDAO`, `AdminDepartmentService`, `AdminDepartmentDAO` | 관리자 의사 등록/수정, 의사 스케줄 관리, 진료과 등록/수정, 사용자 의료진 소개/진료과 소개 예정 |
...
> 작성 기준: 2026-06-30 KST(+09:00), 현재 작성된 파일과 `web.xml`/`@WebServlet` 매핑 기준으로 정리한다.

### 구현 전

| 영역 | 대상 | 비고 |
|---|---|---|
| 사용자 의료진 소개 | `DoctorService`, `DoctorDAO`, 사용자 의료진 목록/상세 화면 | 관리자 의료진 관리 파일은 있으나 사용자 소개용 Java 모듈과 JSP는 아직 없다. |
| 사용자 진료과 소개 | `DepartmentService`, `DepartmentDAO`, 사용자 진료과 목록/상세 화면 | 관리자 진료과 관리 파일은 있으나 사용자 소개용 Java 모듈과 JSP는 아직 없다. |

### 구현 중

| 영역 | 대상 | 현재 상태 |
|---|---|---|
| 회원/인증/마이페이지 | `LoginService`, `MemberRegisterService`, `FindAccountService`, `UserMyPageService`, `UpdateUserInfoService`, member controller/JSP | 로그인, 회원가입, 아이디/비밀번호 찾기, 마이페이지, 정보수정, 탈퇴, 로그아웃, 예약취소 흐름은 Servlet 매핑이 추가되어 있다. 다만 `views/member/process/*.jsp`와 일부 JSP 내 Service 호출이 남아 정리가 필요하다. |
| 관리자 회원 관리 | `AdminMemberService`, `AdminMemberDAO`, `AdminMemberListServlet`, `AdminMemberDetailServlet`, 관리자 회원 JSP | 목록/상세 조회와 상세 화면 메모 조회 구조는 있으나 권한 검증과 error handling 보완이 필요하다. 상세 진입은 `@WebServlet("/admin/member/detail")`로 연결되어 있다. |
| 관리자 예약 관리 | `AdminAppointmentService`, `AdminAppointmentDAO`, `AdminAppointmentListServlet`, `AdminAppointmentStatusServlet`, `adminAppointmentList.jsp` | 예약 목록 조회와 상태 변경 Servlet은 있으나 transaction/error handling 보완이 필요하다. |
| 관리자 의료진 관리 | `AdminDoctorService`, `AdminDoctorDAO`, `AdminDoctorListServlet`, `AdminDoctorFormServlet`, 의료진 JSP | Servlet 기반 목록/폼 매핑이 있으나 기존 process JSP와 JSP 내 Service 호출이 함께 남아 있고 DAO error handling, validation 정리가 필요하다. |
...
| 영역 | 대상 | 완료 기준 |
|---|---|---|
| 공통 DB 연결 | `DBConnection`, `META-INF/context.xml`, `WEB-INF/web.xml` `resource-ref` | `java:comp/env/jdbc/hospitalDB` JNDI DataSource 기반 DB 연결과 JDBC 자원 정리 유틸이 작성되어 있다. |
...
| 병원 소개/오시는 길 | `intro.jsp`, `location.jsp`, `hospital-location.js` | `/hospital/intro.do`, `/hospital/location.do`가 JSP 매핑으로 연결되어 있고 오시는 길 화면은 카카오 지도 API 키 설정을 사용한다. |
| 관리자 메모 관리 | `AdminMemoService`, `AdminMemoDAO`, `AdminMemoSaveServlet`, `AdminMemoDeleteServlet`, `memberDetail.jsp` | 회원 상세 화면에서 메모 목록 조회, 등록, 삭제 흐름이 작성되어 있다. |
...
| 중간 | 관리자 의료진 process JSP 제거 | `adminDoctorListViewProcess.jsp`, `adminDoctorDetailProcess.jsp`와 `adminDoctorDetail.jsp`의 JSP 내 Service 호출을 Servlet 기반 흐름으로 정리해야 한다. |
